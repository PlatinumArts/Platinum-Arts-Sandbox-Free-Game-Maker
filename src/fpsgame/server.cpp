...
namespace server
{
    ...
    struct clientinfo
    {
        ...
        ENetPacket *getdemo, *getmap, *getcfg, *clipboard;
        int lastclipboard, needclipboard;

        clientinfo() : getdemo(NULL), getmap(NULL), getcfg(NULL), clipboard(NULL) { reset(); }
...
#ifndef STANDALONE
    ICOMMAND(modename, "i", (int *n), result(modename(*n, "unknown")));
#endif
    ...
    static void freegetcfg(ENetPacket *packet)
    {
        loopv(clients)
        {
            clientinfo *ci = clients[i];
    ...
    int checktype(int type, clientinfo *ci)
    {
        ...
        static const int servtypes[] = { N_SERVINFO, N_INITCLIENT, N_WELCOME, N_MAPRELOAD, N_SERVMSG, N_DAMAGE, N_HITPUSH, N_SHOTFX, N_EXPLODEFX, N_DIED, N_SPAWNSTATE, N_FORCEDEATH, N_ITEMACC, N_ITEMSPAWN, N_TIMEUP, N_CDIS, N_CURRENTMASTER, N_PONG, N_RESUME, N_BASESCORE, N_BASEINFO, N_BASEREGEN, N_ANNOUNCE, N_SENDDEMOLIST, N_SENDDEMO, N_DEMOPLAYBACK, N_SENDMAP, N_SENDCFG, N_DROPFLAG, N_SCOREFLAG, N_RETURNFLAG, N_RESETFLAG, N_INVISFLAG, N_CLIENT, N_AUTHCHAL, N_INITAI, N_EXPIRETOKENS, N_DROPTOKENS };
        if(ci)
        {
    ...
    void receivefile(int sender, uchar *data, int len)
    {
        ...
        switch(type)
        {
            case N_UPLOADCFG:
                DELETEP(cfgdata);
                if(!len) return;
                cfgdata = opentempfile("cfgdata", "w+b");
                if(!cfgdata) { sendf(sender, 1, "ris", N_SERVMSG, "failed to open temporary file for cfg"); return; }
                cfgdata->write(data, len);
                sendf(sender, 1, "ris", N_SERVMSG, "received cfg file");
                break;
            case N_UPLOADMAP:
                DELETEP(mapdata);
                if(!len) return;
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            case N_POS:
            {
                ...
                if(cp)
                {
                    ...
                    {
                        //if(!ci->local && !m_edit && max(vel.magnitude2(), (float)fabs(vel.z)) >= 180) // PAS - don't enforce speeds, we allow per map configuration and this conflicts
                            //cp->setexceeded();
        ...
        case N_TEXT:
        {
            ...
            if(mtype&CHAT_TEAM)
            {
                ...
                loopv(clients)
                {
                    ...
                    sendf(t->clientnum, 1, "riiis", N_TEXT, cq->clientnum, mtype, text);
                }
                if(mtype&CHAT_EMOTE)
                {
                    print("(%s) *%s(%i) %s", ci->team, colorname(ci), ci->clientnum, text);
                }
                else
                    print("(%s) <%s(%i)> %s", ci->team, colorname(ci), ci->clientnum, text);
            }
            else //it would probably be TEXT... otherwise
            {
                ...
                {
                        clientinfo *t = clients[i];
                    if(t==ci || t->state.aitype != AI_NONE) continue;
                    sendf(t->clientnum, 1, "riiis", N_TEXT, ci->clientnum, mtype, text);
                }
                if(mtype&CHAT_EMOTE)
                {
                ...
                if(strcmp(ci->team, text) && m_teammode && (!smode || smode->canchangeteam(ci, ci->team, text)))
                  {
                    ...
                    print("%s(%i) changed teams to %s", ci->name, ci->clientnum, ci->team);
                }
                break;
            }
            ...
            case N_MAPCHANGE:
            {
                ...
                print("%s(%i) voted for \"%s\" on \"%s\"", ci->name, ci->clientnum, modename(reqmode, "unknown"), text);
            ...
            case N_GETMAP:
                if(!mapdata || !cfgdata) sendf(sender, 1, "ris", N_SERVMSG, "no map to send");
                else if(ci->getmap || ci->getcfg) sendf(sender, 1, "ris", N_SERVMSG, "already sending map");
                else
                {
                    sendf(sender, 1, "ris", N_SERVMSG, "server sending map...");

                    if((ci->getcfg = sendfile(sender, 2, cfgdata, "ri", N_SENDCFG)))
                        ci->getcfg->freeCallback = freegetcfg;
            ...
            case N_TEXTUREREQUEST:
            {
                ...
                if(file)
                {
                    ...
                    sendf(sender, 1, "ris", N_SERVMSG, texmsg);
                    sendfile(sender, 2, file, "risi", N_SENDTEXTURE, text, slot);
                }
                else
                {
                    defformatstring(err)("server - texture: %s not found", text);
                    sendf(sender, 1, "ris", N_SERVMSG, err);
                } */
                break;
            }
