...
namespace game
{
    ...
    struct movable : dynent
    {
            ...
            stackpos(0, 0, 0)
        {
            state = CS_ALIVE;
            type = ENT_INANIMATE;
            yaw = float((e.attr[0]+7)-(e.attr[0]+7)%15);
        ...
        void hitpush(int damage, const vec &dir, dynent *actor, int gun)
        {
        ...
        void explode(dynent *at)
        {
            state = CS_DEAD;
            exploding = 0;
            //game::explode(true, (dynent *)at, o, this, guns[GUN_BARREL].damage, GUN_BARREL);
        }

        ///TODO: enable explosing by trigger
        void damaged(int damage, dynent *at, int gun = -1)
        {
...
//            else explode(at);
    ...
    void hitmovable(int damage, movable *m, dynent *at, const vec &vel, int gun)
    {
