...
//        {
//            MapEntities::StaticEntity &f = (MapEntities::StaticEntity &)e;
...
//        else e.flags = 0;
        if(!local)
            game::addmsg(SV_EDITENT, "riii3i2v", i, (int)(e.o.x*DMF), (int)(e.o.y*DMF), (int)(e.o.z*DMF), e.type, e.attr.length(), e.attr.length(), e.attr.getbuf());
      }
...
namespace game
{
    ...
    {
        Character::CharacterInfo& charinfo = player1->getcharinfo();
...
//        {
//            Control::ControlNPC* ca = dynamic_cast<Control::ControlNPC*>(Control::controls[i]);
//            if(ca) { ca->disconnect(); }
//        }
    ...
    ICOMMAND(flying, "", (), intret(player1->flying = !player1->flying));
    ...
    {
        DynamicEntity *d = getclient(cn);
        Character::CharacterInfo& info = d->getcharinfo();
        return d ? info.getplayermodel() : -1;
    }
    ICOMMAND(getclientmodel, "i", (int *cn), intret(getclientmodel(*cn)));
    ICOMMAND(getplayermodel, "", (), intret(getclientmodel(player1->clientnum)));
    ...
    SVARP(localmap, "moviecubedemo");
    VARP(lobbymode, STARTGAMEMODE, 1, NUMGAMEMODES );
    SVARP(lobbymap, "moviecubedemo");
...
//        }
    ...
    void sendintro()
    {
        ...
        sendstring(hash, p);
        Character::CharacterInfo& info = player1->getcharinfo();
...
//                conoutf("%s", dbg);
                ...
                {
                    Control::ControlRemote* control = new Control::ControlRemote;
            ...
            case SV_SWITCHMODEL:
            {
                ...
                if(d)
                {
                	Character::CharacterInfo& charinfo = d->getcharinfo();
...
//                conoutf("DEBUG: game::parsemessages SV_CDIS cn: %d", n);
                loopv(Control::controls)
                {
                    if(Control::controls[i]->dynent->clientnum == n)
                    {
                        Control::Control* c = Control::controls[i]->cleanup();
                        if(c) delete c;
...
// TODO (offtools#1#): move spawn related stuff to cmode
                if(s->controltype == CONTROL_NPC && editmode)
                {
...
//                    conoutf("DEBUG: game::parsemessages 2. SV_RESUME cn: %d, state: %d", cn, d->state);
            ...
            {
//                int i = getint(p);
//                if(!entities::getents().inrange(i)) break;
//                //entities::setspawn(i, true);
//                playsound(S_ITEMSPAWN, &entities::getents()[i]->o);
//                const char *name = entities::itemname(i);
//                if(name) particle_text(entities::getents()[i]->o, name, PART_TEXT, 2000, 0x32FF64, 4.0f, -8);
                break;
            }
...
//                else if(t==I_BOOST) { playsound(S_V_BOOST10); conoutf(CON_GAMEINFO, "\f2+10 health will spawn in 10 seconds!"); }
            ...
            case SV_INITBOT:
            {
                ...
                if(!b) break;
                b->clienttype = (cltype)ct;
                b->ownernum = on;

                Character::CharacterInfo& info = b->getcharinfo();
...
//                conoutf("%s", dbg);
                ...
                if(player1->clientnum == b->ownernum)
                {
                	...
                	{
                	    Control::ControlNPC* ca = dynamic_cast<Control::ControlNPC*>(Control::controls[i]);
                		if(ca && !Control::controls[i]->getstate())
                		{
//                            conoutf("DEBUG: game::parsemessages SV_INITBOT init ControlNPC");
                        	ca->connect(b);
                			break;
                		}
                	}
...
//                    conoutf("DEBUG: game::parsemessages SV_INITBOT init ControlRemote");
                    Control::controls.add(new Control::ControlRemote(b));
                }
                break;
            }
